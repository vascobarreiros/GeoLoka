...
class GetAllDevicesDetails: ObservableObject {
    ...
    init() {
        let url = URL(string: "https://lokaiosapp-cnkwoooqra-ew.a.run.app/get_locations_new.php?identifier=\(self.identifier)&hours=\(self.delta_hours)")!
...
class SpecificDeviceDetails: ObservableObject {
    ...
    init() {
        let url = URL(string: "https://lokaiosapp-cnkwoooqra-ew.a.run.app/get_locations_with_double_entry.php?device=device\(self.device)&hours=\(self.delta_hours)")!
        URLSession.shared.dataTask(with: url) {(data, response, error) in
             do {
                 if let d = data {
                    let decodedData = try JSONDecoder().decode([loka_result].self, from: d)
...
class FetchAllLoka_Data: ObservableObject {
    ...
    init() {
        let url = URL(string: "https://lokaiosapp-cnkwoooqra-ew.a.run.app/get_locations.php")!
        URLSession.shared.dataTask(with: url) {(data, response, error) in
             do {
                 if let d = data {
                    let decodedData = try JSONDecoder().decode([loka_result].self, from: d)
